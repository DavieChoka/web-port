      ...
      <div className="container mx-auto px-6">
        ...
        {isMobileMenuOpen && <div className="md:hidden py-4 bg-background/95 backdrop-blur-md rounded-lg mt-2 border border-border">
            <div className="flex flex-col space-y-2 px-4">
              {navItems.map(item => <button key={item.label} onClick={() => scrollToSection(item.href)} className="text-foreground hover:text-primary transition-colors duration-300 font-medium py-2 text-left">
                  {item.label}
                </button>)}
            </div>
          </div>}
